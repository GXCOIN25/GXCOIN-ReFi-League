...
async function getAllFiles(dir, fileList = []) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      if (!file.startsWith('.') && file !== 'node_modules' && file !== 'dist' && file !== 'build' && file !== 'attached_assets') {
        await getAllFiles(filePath, fileList);
      }
    } else {
      if (!file.endsWith('.lock') && !file.endsWith('.tar.gz') && !file.endsWith('.jpg') && !file.endsWith('.png') && stat.size < 5000000) {
